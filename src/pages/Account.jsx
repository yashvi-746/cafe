...
const Account = ({ user, onLogout, setPage, favorites }) => {
    ...
    <div className="pt-28 pb-24 max-w-7xl mx-auto px-5 md:px-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h2 className="nb-display text-3xl mt-1">{user.name}</h2>
        </div>
        <button onClick={onLogout} className="flex items-center gap-2 text-sm nb-text-fade nb-focus"><LogOut size={14}/> Sign out</button>
      </div>
      <div className="flex gap-8">
        <div className="hidden md:flex flex-col gap-1 w-52 shrink-0">
          {tabs.map(t=>(
            <button key={t.k} onClick={()=>setTab(t.k)} className={`nb-focus flex items-center gap-2.5 px-3 py-2.5 text-sm text-left ${tab===t.k ? "nb-bg-ink nb-text-cream" : "nb-text-soft hover:nb-bg-paper2"}`}><t.i size={14}/>{t.l}</button>
          ))}
        </div>
        <div className="flex gap-2 md:hidden overflow-x-auto nb-scroll pb-2 mb-2 w-full">
        ...
        <div className="grow min-w-0">
          {tab==="overview" && (
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="nb-card p-5"><p className="text-xs nb-text-fade">Recent order</p><p className="nb-display text-lg mt-2">{user.orders[0].items.join(", ")}</p><p className="text-xs nb-text-fade mt-1">{user.orders[0].date} · {money(user.orders[0].total)}</p><button onClick={()=>setPage("menu")} className="text-xs nb-underline mt-3 nb-focus">Order Again</button></div>
              <div className="nb-card p-5"><p className="text-xs nb-text-fade">Upcoming reservation</p><p className="nb-display text-lg mt-2">{user.reservations[0].date}</p><p className="text-xs nb-text-fade mt-1">{user.reservations[0].time} · {user.reservations[0].guests} guests · {user.reservations[0].table}</p></div>
            ...
            <div className="space-y-3">
              {user.orders.map(o=>(
                <div key={o.id} className="nb-card p-4 flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-4"><Badge tone="olive">{o.status}</Badge><span className="text-sm">{money(o.total)}</span></div>
                </div>
              ))}
            </div>
          )}
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-3 mt-5">
                {[{n:"Free Coffee",p:500},{n:"Free Dessert",p:400},{n:"₹100 Off",p:300},{n:"Exclusive Seasonal Item",p:600}].map(r=>(
                  <div key={r.n} className="nb-card p-4 flex justify-between items-center"><span className="text-sm">{r.n}</span><span className="text-xs nb-text-fade">{r.p} pts</span></div>
                ))}
              </div>
            </div>
          )}
          {tab==="favorites" && (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {favProducts.length===0 ? <p className="text-sm nb-text-fade">No favorites saved yet — tap the heart on any item.</p> :
                favProducts.map(p=>(
                  <div key={p.id} className="nb-card flex gap-3 p-3">
                    <ProductImg cls={p.img} h="h-16 w-16 shrink-0" />
          ...
          {tab==="coupons" && (
            <div className="space-y-3">
              {user.coupons.map(c=>(
                <div key={c.code} className="nb-card p-4 flex justify-between items-center">
          ...
          {tab==="notifications" && (
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="nb-card p-4 flex justify-between"><span className="text-sm">{n.t}</span><span className="text-xs nb-text-fade shrink-0 ml-3">{n.time}</span></div>
              ))}
            </div>
          )}
          {tab==="profile" && (
            <div className="space-y-3 max-w-sm">
              <input defaultValue={user.name} className="w-full border nb-border px-3 py-2.5 text-sm outline-none nb-focus" />
              <input defaultValue={user.email} className="w-full border nb-border px-3 py-2.5 text-sm outline-none nb-focus" />
              <input defaultValue={user.phone} className="w-full border nb-border px-3 py-2.5 text-sm outline-none nb-focus" />
              <button className="nb-btn nb-btn-primary px-5 py-2.5 text-sm nb-focus">Save Changes</button>
            </div>
          )}
        </div>
      </div>
